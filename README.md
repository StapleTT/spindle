...
- **Invite-only registration** — accounts require a valid `XXXX-XXXX-XXXX` invite code; admin generates codes from the API
...
Browser  ──▶  Express (server.js)
                ...
                └── /api/admin       invite codes, user list
```
...
Navigate to `/auth` in your browser and register. **The first registered user is automatically granted admin privileges.** You'll be prompted for an invite code — generate one by calling the admin API:
...
Or use any HTTP client. The response contains a `XXXX-XXXX-XXXX` hex code ready to use.
...
- **Session-based auth** — sessions are stored server-side in files; cookies are `httpOnly`, `sameSite: lax`, and `secure` in production.
- **Invite-only registration** — new accounts require a one-time `XXXX-XXXX-XXXX` invite code generated by an admin. Codes are permanently revoked after use.
