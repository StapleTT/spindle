...
const Search = (() => {
  ...
  async function _showResults(accountId, field, q) {
    ...
    // Re-render the sidebar with the searched account visually active.
    // Sidebar.renderFor() stores the override so folder-load callbacks also
    // respect it; setActive() clears it when normal navigation resumes.
    Sidebar.renderFor(accountId, 'INBOX');
